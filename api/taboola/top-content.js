// api/taboola/top-content.js - GET top content rankings from Backstage
export default async function handler(req, res) {
	...
	try {
		...
		const tokenResp = await fetch('https://backstage.taboola.com/backstage/oauth/token', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: new URLSearchParams({ client_id: CLIENT_ID, client_secret: CLIENT_SECRET, grant_type:'client_credentials' }) })
		const tokenJson = await tokenResp.json(); if (!tokenResp.ok) return res.status(tokenResp.status).json({ error:'Taboola OAuth failed', details: tokenJson })
		const accessToken = tokenJson.access_token

		const end = new Date(); const start = new Date();
		if (date === 'last_7d') start.setDate(end.getDate() - 7); else if (date === 'last_14d') start.setDate(end.getDate() - 14); else start.setDate(end.getDate() - 30)
		...
		const q = new URLSearchParams({ start_date: startStr, end_date: endStr, format: 'json' })
		const resp = await fetch(`${base}?${q}`, { headers: { 'Authorization': `Bearer ${accessToken}` } })
		const json = await resp.json(); if (!resp.ok) return res.status(resp.status).json({ error:'Taboola items report error', details: json })
		...
		const sorter = { roas:(a,b)=>(b.roas||-1)-(a.roas||-1), conversions:(a,b)=>b.conversions-a.conversions, ctr:(a,b)=>b.ctr-a.ctr, spend:(a,b)=>b.spend-a.spend }[String(rank).toLowerCase()] || ((a,b)=>(b.roas||-1)-(a.roas||-1))
		items.sort(sorter)
