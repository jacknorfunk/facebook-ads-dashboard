    ...
    <script>
        ...
        function applyFilters() {
            ...
            filteredCampaigns = allCampaigns.filter(campaign => {
                // Status filter - improved logic for real Voluum data
                if (statusFilter !== 'all') {
                    const hasTraffic = (campaign.visits || 0) > 0 || (campaign.cost || 0) > 0;
                ...
                if (roasFilter !== 'all') {
                    ...
                    if (roasFilter === 'excellent' && roas <= 2.0) return false;
                    if (roasFilter === 'good' && (roas <= 1.0 || roas > 2.0)) return false;
                    if (roasFilter === 'break_even' && (roas <= 0.5 || roas > 1.0)) return false;
                    if (roasFilter === 'poor' && roas >= 0.5) return false;
                }
                
                // Spend filter - adjusted for real data
                if (spendFilter !== 'all') {
                    ...
                    if (spendFilter === 'low' && (spend <= 1 || spend >= 100)) return false;
                    if (spendFilter === 'zero' && spend > 1) return false;
                }
